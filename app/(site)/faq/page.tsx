...
export default async function FaqPage() {
  ...
  return (
    <PageShell
      ...
      lead={
        <>
          Every question below is answered by TechBrotherz, a walk-in cell phone and computer repair
          shop at {SITE.street} in {SITE.city}, {SITE.region}. If your question is not here, call{" "}
          {SITE.phone} and ask.
        </>
      }
      answerBox={{
        answer: `TechBrotherz in Calgary is a walk-in shop, so no appointment is needed. Most phone screen and battery repairs take about ${waitMinutes} minutes while you wait, every price includes the part and the labour, and every repair carries a ${warrantyDays}-day warranty on both the part and the workmanship.`,
        keyFacts: [
      ...
      <Section aria-labelledby="faq-next-heading">
        ...
        <ul className="type-body divide-tb-border mt-8 grid gap-x-10 divide-y md:grid-cols-2 md:divide-y-0">
          {[
            ...
            { href: "/about", label: "About the shop and what we repair" },
          ].map((link) => (
