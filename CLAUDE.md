...
| Ownership                    | Local, family owned (confirmed by the owner 2026-08; founding year still unknown, so no duration is claimed) |
| Parking                      | Free, in the plaza in front of the store (confirmed 2026-08)      |
...
| ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| **19** | **Parking. ANSWERED 2026-08 by the owner: free parking in front of the store, in the plaza.** The plaza is also a locating fact and is used in the getting-here copy. Carried in full on `/locations/calgary`, one line everywhere else. Space count and any time limit remain unknown and unstated. | Published. Sourced to the owner in `content/local-inventory.md`. |
...
| 8      | **Real Google review count and star rating: the Place ID is the last blocker.** RatingBadge is built and placed (hero stat row and the reviews section), reads the same ReviewSummary that gates `AggregateRating`, and renders nothing until real data arrives; `pnpm verify` now fails on any hardcoded rating or count. One line to the client: send the Google Business Profile link, and either add a Google Cloud billing card and send a restricted API key, or grant project access. The free tier covers it. The moment the data lands, the badge shows whatever the real figure is. | Badge dormant, guard armed, no rating shown or emitted anywhere. |
...
| 12     | **Exact geo coordinates** for the storefront, or confirm we can read them from the Google Business Profile.                                                                                                                                   | **Answered 2026-08**: read from the client's own Maps listing URL (51.0366821, -113.9869842). `geo` now emitted on `LocalBusiness`.                 |
...
| 26     | **The Calgary skyline image (supplied 2026-08 via WhatsApp): who owns it?** It is 1600px wide, below what a full-bleed hero needs, and it appears to be a professionally produced composite, most likely a commercial marketing render rather than a photograph the client took. Confirm the client owns it or holds a licence, or approve sourcing a licensed skyline instead. | **Not used.** The hero keeps the solid black panel until this is answered. |
| 27     | **The logo. ANSWERED 2026-08: the client wants a different mark, and a new one is designed in-house** and shipped: a rounded green square holding a phone with a repair checkmark drawn in ink (dark-on-green per the design rule), beside "TechBrotherz" typeset in the display face with the closing z in green, and the exact descriptor line beneath. Pure SVG and text, no image files, crisp at any size, light and dark variants. The favicon matches. The supplied PNGs stay in `public/` so reverting is a one-line change if the client prefers the old mark after seeing this one. **Client to confirm they are happy with the new mark.** | New mark live. Old PNGs kept for revert. |
