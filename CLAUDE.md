...
| 27     | **The logo: what change is wanted?** The client asked whether the logo can be changed without saying what. The rendering issue could not be reproduced: both committed logo files are transparent PNGs and the header renders clean, so a pale box on their screen is most likely a cached earlier deploy (hard refresh to confirm). If they want a different mark, that is a design brief. **A transparent SVG or high-resolution PNG original is still outstanding** since the logos arrived. | Current transparent PNGs stay in place. |
